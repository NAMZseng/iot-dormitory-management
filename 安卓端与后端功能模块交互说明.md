-   [安卓端与后端功能模块交互说明](#安卓端与后端功能模块交互说明)
    -   [1 基本信息模块](#基本信息模块)
        ...
        -   [1.3 默认密码修改](#默认密码修改)
        -   [1.4 手机号修改](#手机号修改)
        -   [1.5 密码找回（附加功能）](#密码找回附加功能)
    -   [2 学生信息模块](#学生信息模块)
        -   [2.1 基本信息展示](#基本信息展示)
        -   [2.2 信息更新（附加功能）](#信息更新附加功能)
    -   [3 人流管理模块](#人流管理模块)
        -   [3.1 每日出入刷卡图表显示](#每日出入刷卡图表显示)
        -   [3.2 异常出入信息显示](#异常出入信息显示)
        -   [3.3 学生未归寝提醒](#学生未归寝提醒)
    -   [4 环境监控模块](#环境监控模块)
        -   [4.1 每日温湿度图表显示](#每日温湿度图表显示)
        -   [4.2 温湿度异常信息提醒](#温湿度异常信息提醒)
...
get请求给服务端，服务端通过查询staff\_info表，判断手机号与密码是否匹配。若匹配，则返回该封装了该职工信息的**类对象**；若不匹配否则返回**null**；
...
get请求传给服务端，服务端会根据宿舍楼号查询student\_info表，将该楼的所有学生信息，以一个**列表List<StudentInfo>，通过Json格式**，返回给返回给安卓端。安卓端可以以**瀑布流**的布局展示学生信息。

student\_info各字段信息如下：
...
当宿舍管理员点击该部分的按钮时，安卓端将登陆职工的所在的**宿舍楼号**（buildingNum），通过http
get请求传给服务端，服务端通过查询温湿度数据humiture\_info表，将**该楼的该日**的温湿度以一个**列表List<HumitureInfo>，通过Json格式**，返回给返回给安卓端。安卓端可以**折线图**等形式展现。为方便测量，温湿度默认是**每分钟**采集一次，折线图的单位可按**每小时**进行模拟。

humiture\_info表的各个字段如下：
